...
def data_analysis():
    project_dir = Path(__file__).parent / "downloaded_csv_files"
    if not project_dir.exists():
        project_dir = Path(__file__).parent / "test_csv_files"  # katalog z testowymi plikami
        print("\nNIE ZNALEZIONO FOLDERU \"downloaded_csv_files\" -> ANALIZA NA TESTOWYCH PLIKACH")

    rotacje_files = [file for file in project_dir.glob("rotacje_*.csv")]
    stany_files = [file for file in project_dir.glob("stany_*.csv")]
    product_files = [file for file in project_dir.glob("products_list*.csv")]

    try:
        if len(rotacje_files) == 0:
            raise FileNotFoundError(f"Brak plików zawierajacych rotacje_.csv w folderze {project_dir}")

        if len(stany_files) == 0:
            raise FileNotFoundError(f"Brak plików zawierajacych stany_.csv w folderze {project_dir}")
    ...
    #====================================== PLIK GŁÓWNY ========================================================
    output_analysis_file = out_dir / f"analysis_for_stany_{start_stany_str}_rotacje_{start_rotacje_str}_at_{now_str}.csv"
    ...
    #====================================== PLIK UPROSZCZONY ====================================================
    output_stats_file = out_dir / f"stats_{now_str}.csv"
